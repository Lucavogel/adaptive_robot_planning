...
class LynxWorkspaceAnalyzer:
    def __init__(self, urdf_path=None):
        ...
        # Initialiser ROS (optionnel si MoveIt n'est pas disponible)
        try:
            rospy.init_node("lynx_workspace_analyzer", anonymous=True)
            self.ros_available = True
            print("✅ ROS initialisé")
        except:
            self.ros_available = False
            print("⚠️  ROS non disponible - Mode analyse URDF uniquement")
        ...
        # Initialiser MoveIt si disponible
        if self.ros_available:
            try:
                ...
                print("✅ MoveIt initialisé")
            except Exception as e:
                self.moveit_available = False
                print(f"⚠️  MoveIt non disponible: {e}")
        else:
            self.moveit_available = False
    ...
    def get_default_lynx_info(self):
        """Informations par défaut du Lynx SES900 basées sur le modèle standard"""
        return {
            'name': 'lynx_ses900',
            'links': ['base_link', 'shoulder_link', 'upper_arm_link', 'forearm_link', 'wrist_link', 'hand_link', 'tool_link'],
            'joints': ['joint_1', 'joint_2', 'joint_3', 'joint_4', 'joint_5', 'joint_6'],
            'joint_limits': {
                'joint_1': {'lower': -3.1416, 'upper': 3.1416},  # Base rotation
                'joint_2': {'lower': -3.1416, 'upper': 3.1416},  # Shoulder
                'joint_3': {'lower': -3.1416, 'upper': 3.1416},  # Elbow
                'joint_4': {'lower': -3.1416, 'upper': 3.1416},  # Wrist roll
                'joint_5': {'lower': -3.1416, 'upper': 3.1416},  # Wrist pitch
                'joint_6': {'lower': -3.1416, 'upper': 3.1416}   # Wrist yaw
            },
            'dh_parameters': [
    ...
    def calculate_forward_kinematics(self, joint_angles):
        ...
        joint_angles: liste de 6 angles en radians
        """
        ...
        for i, (angle, dh) in enumerate(zip(joint_angles, self.robot_info['dh_parameters'])):
            ...
            if rot[0] != 0:  # Rotation autour de X
                ...
                T_local[0:3, 0:3] = R_x @ T_local[0:3, 0:3]
            
            if rot[1] != 0:  # Rotation autour de Y
                R_y = np.array([[np.cos(rot[1]), 0, np.sin(rot[1])],
                               [0, 1, 0],
                               [-np.sin(rot[1]), 0, np.cos(rot[1])]])
                T_local[0:3, 0:3] = R_y @ T_local[0:3, 0:3]
            
            if rot[2] != 0:  # Rotation autour de Z
                R_z = np.array([[np.cos(rot[2]), -np.sin(rot[2]), 0],
                               [np.sin(rot[2]), np.cos(rot[2]), 0],
                               [0, 0, 1]])
                T_local[0:3, 0:3] = R_z @ T_local[0:3, 0:3]
            
            # Rotation du joint
            if axis[0] != 0:  # Rotation autour de X
                R_joint = np.array([[1, 0, 0],
                                   [0, np.cos(angle), -np.sin(angle)],
                                   [0, np.sin(angle), np.cos(angle)]])
            elif axis[1] != 0:  # Rotation autour de Y
                R_joint = np.array([[np.cos(angle), 0, np.sin(angle)],
                                   [0, 1, 0],
                                   [-np.sin(angle), 0, np.cos(angle)]])
            else:  # Rotation autour de Z
                R_joint = np.array([[np.cos(angle), -np.sin(angle), 0],
    ...
    def analyze_workspace_envelope(self, points):
        ...
        analysis = {
            ...
            'volume': {
                ...
                'workspace_efficiency': 0.0  # À calculer avec un volume plus précis
            }
        }
    ...
    def visualize_workspace(self, points, save_path=None):
        ...
        ax1.scatter(points[:, 0], points[:, 1], points[:, 2], alpha=0.1, s=1)
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ax1.set_zlabel('Z (m)')
        ...
        ax2.scatter(points[:, 0], points[:, 1], alpha=0.1, s=1)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ...
        ax3.scatter(points[:, 0], points[:, 2], alpha=0.1, s=1)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Z (m)')
    ...
    def save_analysis(self, points, analysis, filename="lynx_workspace_data.json"):
        ...
        data = {
            ...
            'timestamp': rospy.Time.now().to_sec() if self.ros_available else 0,
    ...
    def run_complete_analysis(self, num_samples=10000):
        ...
        # 3. Visualisation
        self.visualize_workspace(points, "lynx_workspace.png")
        
        # 4. Sauvegarde
        self.save_analysis(points, analysis)
...
def main():
    ...
    points, analysis = analyzer.run_complete_analysis(num_samples=10000)
    
    print(f"\n🎉 Analyse terminée!")
    print(f"📊 {len(points)} points analysés")
    print(f"📁 Données sauvegardées dans lynx_workspace_data.json")
    print(f"📈 Graphique sauvegardé dans lynx_workspace.png")
